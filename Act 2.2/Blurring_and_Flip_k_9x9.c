...
void main()
{
  ...
  outputImage_BLUR = fopen("gtr35-HD.bmp_out_9x9.bmp", "wb"); // Imagen transformada
  //outputImage_BLUR = fopen("gtr35-4K.bmp_out_9x9.bmp", "wb"); // Imagen transformada
  long ancho;
  long alto;
  ...
  {
    xx[i] = fgetc(image);
    fputc(xx[i], outputImage_BLUR); // Copia cabecera a nueva imagen
  }
...
#pragma omp parallel
  ...
  // Matriz de 9x9
  //Para modificar el ancho de la ventana, se modifica la linea:
  ...
  #pragma omp parallel
  {
    ...
    for (int i = 0; i < alto * ancho; i++)
    {
      ...
      {
        fputc(tmp_pixel_vector[i], outputImage_BLUR);
        fputc(tmp_pixel_vector[i], outputImage_BLUR);
        fputc(tmp_pixel_vector[i], outputImage_BLUR);
      }
      else
    ...
    //   if (linea <= 3 || (i == (inicioLin) && i <= (inicioLin + 3)) || (i >= (finLin - 3) && i == (finLin)) || linea >= (alto - 4))
    //   {
    //     fputc(tmp_flip_V_vector[i], outputImage_BLUR);
    //     fputc(tmp_flip_V_vector[i], outputImage_BLUR);
    //     fputc(tmp_flip_V_vector[i], outputImage_BLUR);
    //   }
    //   else
